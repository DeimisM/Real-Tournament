...

{
...

    {
        ...
        if ((float)health.hp / health.hp  <= lowHpLimit)

        {
            ...
            vig.intensity.value = (lowHpLimit - (float)health.hp / health.hp) / lowHpLimit * maxVignetteValue;

            chr.active = true;

            chr.intensity.value = (lowHpLimit - (float)health.hp / health.hp) / lowHpLimit * maxAberrationValue;

            lgg.active = true;

            lgg.gain.Override(new Vector4(1f, 1f, 1f, -((lowHpLimit - (float)health.hp / health.hp) / lowHpLimit * (1f - maxGainValue))));
